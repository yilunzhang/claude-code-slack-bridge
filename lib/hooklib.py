...
allowlist + 身份门 + 凭据版本门 + session 三元组门控的直发)给绑定会话发 @owner 告警
(见 stop_failure_entry;fail-closed,exit 0,不写心跳)。
出站分块:`util.chunk_text_with_footer(body, footer, constants.CHUNK_LIMIT)`(contracts §2.1)——
每块(含页脚的末块)≤ CHUNK_LIMIT,页脚只在末块、绝不丢正文。"""
import datetime
import os
import re
import sqlite3
...
def _touch_hook_heartbeat(event):
    ...
    pkg_root(防「另一 install/旧版本/仅 SessionEnd」假阳性)。绝不抛异常(hook 永不因此失败)。"""
    try:
...
def session_end_entry(payload, conn=None, prober=None, clock=None, start_pid=None):
    ...
    try:
        if conn is None:
            ...
            # SessionEnd 关绑定是 best-effort(daemon cc_gone 兜底);更短的等锁超时 → 锁竞争时快速让路。
...
def _bump_drop_counter():
    try:
        # 可观测计数用**短等锁**(BUSY_TIMEOUT_OBS_MS):计数只是观测,拿不到锁就算了,不拖住 CC 退出。
...
def _fail_closed_drop(exc, hook="stop"):
    ...
    if not logged and not counted:
        ...
        print("slack-bridge: hook fail-closed drop (observability degraded)",
              file=sys.stderr)


def _turn_footer(payload):
    """每轮转发页脚(context/model/effort)= best-effort 装饰,完全隔离于转发关键路径(fail-open 铁律):
    延迟导入 → 即便 ctxmeter 损坏也只丢页脚、不炸 hook;任何异常 / 非 str 返回 → ''。"""
    try:
...
def run_stop_hook(payload, *, conn, prober, clock, start_pid=None):
    ...
    if inst is None:
        return {"suppressed": True, "reason": "no-instance"}  # 运行时失败 → fail-closed
    pid, lstart = inst

    # ① 抑制链闩:续写 Stop 全抑制;fresh Stop 先关闩再正常处理
    ...
    # 分块 + 页脚(contracts §2.1):页脚只在末块,每块 ≤ CHUNK_LIMIT,正文一个字符都不丢;
    # 页脚失败 → '' → 等价纯分块(原样转发)。CHUNK_LIMIT 每次读 constants(可被测试覆盖)。
    footer = _turn_footer(payload)
    chunks = util.chunk_text_with_footer(msg, footer, constants.CHUNK_LIMIT)
    group = util.new_id()
    with db.tx(conn):
...
def _handshake(conn, pend, session_id, now):
    ...
    try:
        ...
        # b_sess 冲突:该 session 已绑他处 → close starting + pending failed + 失败通知
        with db.tx(conn):
            if db.cas(conn,
                      "UPDATE pending_bind SET state='failed', latch_open=1 "
                      "WHERE request_id=? AND state='pending'", (rid,)):
...
# StopFailure(与 Stop 互斥,每轮至多一次)。经 lib.notify.run_notify 给**本 session 绑定会话**发一条
# @owner 告警(Block Kit),与 notify skill 同格式、同门控。未绑定 session 静默(如实不发)。
# 告警正文含**有界的** error/error_details/cwd 路径,发进可信会话(个人轻量工具假设)。
# fail-closed:一切异常吞掉、exit 0,绝不阻塞 CC。

_MAX_BODY_LEN = 800


def _sanitize_field(val, cap):
    """把一个动态字段净化成**可安全放进 markdown 块的单行文本**。
    字段来自 CC 的 StopFailure payload(本机错误类型/详情/cwd),非外部输入;有意不加 markdown 转义层
    (内联渲染跑偏可接受)。**安全性质由步③ 的 `<!` 中和保证**(Slack 广播 mention 前缀,markdown 块会
    真渲染 <!channel>/<!here>),使 lib.notify.BROADCAST_RE 不命中 → 告警不被整条丢弃。
    顺序关键:**先删毒字符(NUL/孤立 surrogate/其它控制符)、再中和 `<!`** —— 否则 `<\\x00!` /
    `<\\ud800!` 删毒后会还原成 `<!`,漏过。步骤:
      ① 删 NUL 与孤立 surrogate;CR/LF/Tab→空格;其它 C0/DEL 删。
      ② 折叠连续空白为单空格 + strip(保证一行)。
      ③ 中和 `<!` → `< !`。
      ④ 截断到 cap(留 `…`)。
    结果**绝不含** NUL/换行/孤立 surrogate,且 BROADCAST_RE 不命中。"""
    s = str(val)
    chars = []
    for ch in s:
        o = ord(ch)
...
def compose_stop_failure_message(payload):
    ...
    **不含 "已重试耗尽"**(用户选"所有错误类型",含非重试类如 auth/model-not-found)。@owner 前缀由
    run_notify 自动加(独立 section 块),此处**只出正文**,且保证正文永不含 `<!`/NUL/换行。"""
...
def run_stop_failure(payload, *, environ=None, prober=None, start_pid=None,
                     notify_fn=None, make_client=None):
    """组装告警正文 + 只用 payload 的 session_id,经 run_notify 发到本 session 绑定会话。→ 观测摘要 dict。
    **env**:`os.environ if environ is None`(非 `or`,免 environ={} 回退)→ 拷贝 → **先 pop 继承的
    CLAUDE_CODE_SESSION_ID**(payload 缺 sid 时若留着继承值会误发到当前绑定会话)→ 仅当 payload 有合法
    str sid 才注入。lib.notify 惰性导入(不扩 Stop/SessionEnd 的 import 面)。
    `make_client(tokens, version, cooldown_store)`:生产 = notify.default_make_client。"""
    from . import notify  # lazy import
    sid = payload.get("session_id")
    ...
    if make_client is None:
        make_client = notify.default_make_client
...
def _log_stop_failure_outcome(res):
    """观测:告警 sent≠true 时落 hook_drops.log 一行(**payload-free**,只含固定 reason 枚举;
...
def stop_failure_entry(payload, *, prober=None, start_pid=None, notify_fn=None, make_client=None):
    """StopFailure hook fail-closed 入口。**不写 hooks 心跳**:StopFailure 仅 API 错误时触发(罕见)、
    非 hooks 生效的可靠信号;bind/preflight 只认 stop 心跳。一切异常 → _fail_closed_drop + 抑制
    (exit 0,绝不阻塞 CC)。"""
    try:
        res = run_stop_failure(payload, prober=prober, start_pid=start_pid,
                               notify_fn=notify_fn, make_client=make_client)
