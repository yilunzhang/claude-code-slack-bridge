...
| R1-M6 | `DOWNLOAD_DEADLINE_S` 是**整条消息**物化的共享绝对截止(`materialize` 进下载循环时记一次 `deadline_at`,每个 worker 只拿剩余秒数;剩余 ≤ 0 → 瞬态 None),而非 §5.3/§6 字面上「每个文件」各一个;`materialize(..., heartbeat=None)` 多一个可选 kwarg,每个文件之后调用(Inbound 传自己的心跳) | 每文件各占满 90s 时两个 80s 的文件就越过 `ctl.HUNG_THRESHOLD_MS`(150s)被误判挂死;共享截止 + 文件间心跳后两次心跳之间最长 = 90s + 2s 宽限。守卫 `tests/test_media.py::test_shared_deadline_across_files_clock_driven` / `test_hung_threshold_covers_shared_deadline` |
| R1-m1 | `Recovery.slow_tick` **不再**调用 `inbound.drive_materializing_rows`(方法已删);materializing 行的唯一调度入口 = 主循环 `DaemonCore.run_followups → Inbound.drive_pending_rows(budget)` | 主循环已按预算物化一次,slow_tick 再领一份预算 → 单 tick 下载 2 条(§1.2 上限 1)。§1.2「recovery 的重驱走同一预算」现在由「只有一个入口」平凡满足。守卫 `tests/test_daemon_plumbing.py::TestFullLoopRealInboundRecovery`、`tests/test_recovery.py::test_slow_tick_never_downloads` |
| R1-M4 | notify / StopFailure 直发门的读取顺序:**先**取 tokens.json 快照,再用**一条 SELECT**(`db.get_states`,同一读快照)联合读 `outbound_gate` + `outbound_gate_tokens_version` + `tokens_version_seen`,在这一元组上判定(gate 非 ok → `gate-degraded`;版本 ≠ 文件 → `credentials-unverified`) | §7 只写了「读文件版本并要求 == 版本 ∧ gate=="ok"」,没写读取时序;分三次自动提交读会拼出「旧版本的 ok + 新版本号」这个从未存在过的组合,把 daemon 已判 `mismatch` 的凭据放行。守卫 `tests/test_notifyctl.py::test_interleaved_rotation_and_gate_update_fails_closed` / `test_gate_and_version_read_in_one_statement_after_token_snapshot` |
| R2-M1 | §2.6「不可解析」的精确定义:核验的每一页由 `outbound.verify_page(data)` 严格校验 —— `messages` 必须是 list 且元素**全为 dict**;`has_more` 若存在必须是 bool;`response_metadata` 若存在必须是 dict,其 `next_cursor` 若存在必须是 str(空串 = 无下一页,与缺省等价);`has_more=true` 必须带非空 cursor。任一不满足 → error 分支(不翻页、**不计 absent**) | 之前只校验 `messages` 是 list:`{"messages":[null]}`、`next_cursor: 42` 都被折叠成"完整且没有下一页"→ absent → 三次后重发。原则:字段**缺省**取默认,字段**存在但类型错**一律 error。守卫 `tests/test_outbound.py::TestVerify::test_malformed_page_is_error_never_absent` / `test_empty_cursor_with_has_more_false_is_complete_page` / `test_page_validator_pure_function` |
| R2-M5 | §4.5 的 `consumer_socket_last_status / _ready / _last_exit_rc` 与 §9 的 consumer stderr:入库/回调前**先遮蔽再截断**。`make_status_writer(conn, log, secrets_provider=None)` 是唯一状态入库入口,`ConsumerManager(..., secrets_provider=None)`(§8 签名尾随可选 kwarg)在 `_feed` 里对每一行 stderr 先 `util.redact_secrets`;consumer 给 `slack_sdk` logger 树挂 `_SdkLogHandler`(`[sdk] <LEVEL> <logger>: <msg>`,WARNING+,不 propagate,exc_info 只留类型名)—— sdk 自身日志不再经 lastResort 裸打 | 之前 `_feed` 原文交 on_status,`make_status_writer` 原文截断入库,`bridgectl status` 再原样输出:daemon.log 已遮蔽而 status 仍含完整 xapp token。daemon 的遮蔽表随 gate 看到的版本变化刷新(`refresh_secrets_if_rotated`,旧值保留)。守卫 `tests/test_secrets.py::TestNoLeakPaths::test_consumer_stderr_token_never_reaches_status_report` / `test_consumer_manager_scrubs_stderr_before_callbacks` / `test_consumer_sdk_log_handler_redacts_in_process`、`tests/test_consumer.py::test_sdk_logger_output_is_redacted_at_source` |
| R2-M8 | §7 只写了 `tick()` 的「签名变 → load_tokens → reload → auth.test」;`FingerprintGate.startup()` 现在走**同一条**完整路径:先 stat 签名再 `load_tokens`;读失败(ConfigError)→ `degraded:tokens_error`(版本键 = client 内存版本),**不**用内存旧凭据 auth.test、不写 ok,签名不作缓存基线(`reload_pending` → 下一 tick 重读);文件版本 ≠ client 内存版本(构造与启动之间轮换)→ `client.reload_tokens` 后再 auth.test,gate 版本 = client 实际持有的版本,旧版本的 probe 结论失效;reload 后 client 版本仍与文件不符 → degraded:tokens_error(fail-closed) | 之前 startup 吞掉 ConfigError、保存新签名、用旧内存凭据 auth.test 写 ok,后续 tick 因签名相同再也不读文件(chmod 644 / 启动期轮换都被永久错过)。守卫 `tests/test_fingerprint.py::TestStartup::test_startup_unreadable_tokens_degrades_without_caching_signature` / `test_startup_reloads_client_when_file_rotated_after_construction` / `test_startup_never_ok_when_client_cannot_follow_file_version` |
| R2-m1 | `media.describe_files` 反查 `local_path`:按 dest_name(`f<idx>-<id>-<name>`)缺席时回落旧命名(R1-M7 之前发布的目录:净化原名、再是 `<n>-原名` 去重形态),每个路径只认领一次;都找不到仍 `no_url` | 数据目录是新的,现实中没有旧目录人口,但物化的幂等复用(`dest.is_dir()` → 直接返回目录内容)与 payload 反查必须对同一目录给出一致答案:`media_paths` 带着旧路径而 `files[]` 标 `no_url` 是自相矛盾的 payload。守卫 `tests/test_media.py::TestFilePlan::test_describe_files_falls_back_to_legacy_published_names` |
| R2-m3 | §5.6 ① 的取消范围**收紧到契约文本**:approval_card 只在其 `ref_pending_id` 的审批**仍 `pending`**(在 ② 把它们 expired 之前判定)时取消;审批已决(approved / rejected / expired)但 card job 仍 pending / unknown 的,留给 `_guard_ok`(pending 非 pending → cancelled)/ 核验(unknown → hit/absent)收口 | 之前 SQL 取消该绑定**所有** pending/unknown 的 approval_card,比 §5.6「仍 pending 审批的 approval_card」更宽。守卫 `tests/test_lifecycle.py::TestTerminate::test_terminate_cancels_only_cards_of_still_pending_approvals`;`recovery._expire_pendings` 的单条范围不变 |
